...
$vali = '';
					...
					$BusquedaHeader = mysqli_query($link,"SELECT *
                    FROM registro
                    INNER JOIN solicitud
                    ON registro.id_solicitud = solicitud.id_solicitud
                    RIGHT JOIN grupos
                    ...
                    RIGHT JOIN usuarios
                    ON  usuarios.id_usuario = solicitud.id_usuario
					WHERE estatus_generado = 'SI' AND solicitud.num_solicitud_5 = $code AND registro.tipo_solicitud = 'SA' AND registro.telefono != ''");
                        ...
                        $plantilla ='
                          ...
                          RIGHT JOIN usuarios
                          ON  usuarios.id_usuario = solicitud.id_usuario
                          WHERE estatus_generado = 'SI' AND solicitud.id_solicitud = $code AND registro.tipo_solicitud = 'SA' AND registro.telefono != ''")){
                                 while($row = mysqli_fetch_array($BusquedaFooter)){
                                    $plantilla .='<div id="notices">
                                        <div class="tittle">Detalles Especificos:</div>
                                        <div class="notice">'.$row['detalle'].'</div>
...
<body>
  ...
  <textarea id="targetTextArea">
    ...
    $solicitudID = $row['num_solicitud_5'];
    if($vali != $row["num_solicitud_1"]){?>
     ...
     <div><span class="tittle"><b>FECHA: </b>        </span> <span> <?php  echo $row['fecha_num_solicitud_1'] ?></span></div>
