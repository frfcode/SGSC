...
RIGHT JOIN usuarios
ON  usuarios.id_usuario = solicitud.id_usuario
WHERE estatus_generado = 'SI' AND solicitud.num_solicitud_4 = $code AND registro.tipo_solicitud = 'SA' AND registro.telefono != ''";
...
<body>
  ...
  <textarea id="targetTextArea">
    ...
    if($vali != $row["num_solicitud_4"]){
        ...
        $fechaMod = explode("-", $row['fecha_num_solicitud_4']);
        $fechaModVigencia = explode("-", $row['vigencia'])?>
    ...
    <div><span class="tittle"><b>VIGENCIA: </b>     </span> <span> <?php  echo $diff->days .' dias - '.$fechaModVigencia[2].'-'.$fechaModVigencia[1].'-'.$fechaModVigencia[0] ?></span></div>
